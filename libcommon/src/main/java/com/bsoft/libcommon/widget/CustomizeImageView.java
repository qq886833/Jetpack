...
import com.bsoft.libbasic.widget.ViewHelper;
...
public class CustomizeImageView extends AppCompatImageView {
    ...
    public static void setImageUrl(CustomizeImageView view, String imageUrl, boolean isCircle) {
        view.setImageUrl(view, imageUrl, isCircle, 0);
    }

    @BindingAdapter(value = {"image_url", "isCircle", "radius"}, requireAll = false)
    public static void setImageUrl(CustomizeImageView view, String imageUrl, boolean isCircle, int radius) {
        RequestBuilder<Drawable> builder = Glide.with(view).load(imageUrl);
        if (isCircle) {
            builder.transform(new CircleCrop());
        } else if (radius > 0) {
            builder.transform(new RoundedCornersTransformation(DensityUtil.dip2px(radius), 0));
        }
    ...
    public void bindData(int widthPx, int heightPx, int marginLeft, String imageUrl) {
        bindData(widthPx, heightPx, marginLeft, DensityUtil.getScreenWidth(), DensityUtil.getScreenWidth(), imageUrl);
    }
    ...
    private void setSize(int width, int height, int marginLeft, int maxWidth, int maxHeight) {
        ...
        if (params instanceof FrameLayout.LayoutParams) {
            ((FrameLayout.LayoutParams) params).leftMargin = height > width ? DensityUtil.dip2px(marginLeft) : 0;
        } else if (params instanceof LinearLayout.LayoutParams) {
            ((LinearLayout.LayoutParams) params).leftMargin = height > width ? DensityUtil.dip2px(marginLeft) : 0;
        }
